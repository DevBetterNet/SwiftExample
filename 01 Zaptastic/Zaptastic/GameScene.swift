...
//  Zaptastic
//
//  Created by Dev Better on 03/02/2022.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createWave(){
        ...
        let enemyOffsetX: CFloat = 100
        let enymyStartX = 600
        
        if currentWave.enemies.isEmpty {
            for (index, position) in positions.shuffled().enumerated(){
                let enemy = EnemyNode(type: enemyTypes[enemyType], startPosition: CGPoint(x: enymyStartX, y: position), xOffset: CGFloat(enemyOffsetX * CFloat(index * 3)), moveStrainght: true)
                addChild(enemy)
            }
        } else{
            for enemy in currentWave.enemies{
                let node = EnemyNode(type: enemyTypes[enemyType], startPosition: CGPoint(x: enymyStartX, y: positions[enemy.position]), xOffset: CGFloat(enemyOffsetX) * enemy.xOffset, moveStrainght: enemy.moveStraight)
